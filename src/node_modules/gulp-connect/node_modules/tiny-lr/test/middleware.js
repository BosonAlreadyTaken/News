...
function suite(name, app) {return function() {
  ...
  describe('GET /changed', function() {
    ...
    it('with no clients, some files', function(done) {
      request(this.server)
        .get('/changed?files=gonna.style,test.style,it.style')
        .expect('Content-Type', /json/)
        .expect('{"clients":[],"files":["gonna.style","test.style","it.style"]}')
        .expect(200, done);
    });
  });
  ...
  describe('POST /changed', function() {
    ...
    it('with no clients, some files', function(done) {
      var data = { clients: [], files: ['cat.style', 'sed.style', 'ack.js'] };
