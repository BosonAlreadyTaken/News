...
exports['match'] = {
  ...
  'basic matching': function(test) {
    ...
    test.deepEqual(globule.match('*.js', ['foo.js']), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match('*.js', ['foo.js', 'bar.style']), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match(['*.js', '*.style'], 'foo.js'), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match(['*.js', '*.style'], ['foo.js']), ['foo.js'], 'should match correctly.');
    test.deepEqual(globule.match(['*.js', '*.style'], ['foo.js', 'bar.style']), ['foo.js', 'bar.style'], 'should match correctly.');
    test.done();
  },
  'no matches': function(test) {
    test.expect(2);
    test.deepEqual(globule.match('*.js', 'foo.style'), [], 'should fail to match.');
    test.deepEqual(globule.match('*.js', ['foo.style', 'bar.style']), [], 'should fail to match.');
    test.done();
  },
  ...
  'flatten': function(test) {
    test.expect(1);
    test.deepEqual(globule.match([['*.js', '*.style'], ['*.*', '*.js']], ['foo.js', 'bar.style']),
      ['foo.js', 'bar.style'],
...
exports['isMatch'] = {
  'basic matching': function(test) {
    ...
    test.ok(globule.isMatch('*.js', ['foo.js']), 'should match correctly.');
    test.ok(globule.isMatch('*.js', ['foo.js', 'bar.style']), 'should match correctly.');
    test.ok(globule.isMatch(['*.js', '*.style'], 'foo.js'), 'should match correctly.');
    test.ok(globule.isMatch(['*.js', '*.style'], ['foo.js']), 'should match correctly.');
    test.ok(globule.isMatch(['*.js', '*.style'], ['foo.js', 'bar.style']), 'should match correctly.');
    test.done();
  },
  'no matches': function(test) {
    test.expect(6);
    test.ok(!globule.isMatch('*.js', 'foo.style'), 'should fail to match.');
    test.ok(!globule.isMatch('*.js', ['foo.style', 'bar.style']), 'should fail to match.');
    test.ok(!globule.isMatch(null, 'foo.style'), 'should fail to match.');
    test.ok(!globule.isMatch('*.js', null), 'should fail to match.');
    test.ok(!globule.isMatch([], 'foo.style'), 'should fail to match.');
...
exports['find'] = {
  ...
  'basic matching': function(test) {
    ...
    test.deepEqual(globule.find('**/*.js', '**/*.style'),
      ['js/bar.js', 'js/foo.js', 'style/baz.style', 'style/qux.style'],
      'multiple pattern arguments should match.');
    test.deepEqual(globule.find(['**/*.js', '**/*.style']),
      ['js/bar.js', 'js/foo.js', 'style/baz.style', 'style/qux.style'],
      'array of patterns should match.');
    test.deepEqual(globule.find([['**/*.js'], [['**/*.style', 'js/*.js']]]),
      ['js/bar.js', 'js/foo.js', 'style/baz.style', 'style/qux.style'],
  ...
  'unique': function(test) {
    ...
    test.deepEqual(globule.find('**/*.js', '**/*.style', 'js/*.js'), ['js/bar.js', 'js/foo.js',
      'style/baz.style', 'style/qux.style'],
      'file list should be uniqed.');
  ...
  'file order': function(test) {
    test.expect(5);
    var actual = globule.find('**/*.{js,style}');
    var expected = ['style/baz.style', 'style/qux.style', 'js/bar.js', 'js/foo.js'];
    test.deepEqual(actual, expected, 'should select 4 files in this order, by default.');

    actual = globule.find('js/foo.js', 'js/bar.js', '**/*.{js,style}');
    expected = ['js/foo.js', 'js/bar.js', 'style/baz.style', 'style/qux.style'];
    test.deepEqual(actual, expected, 'specifically-specified-up-front file order should be maintained.');

    actual = globule.find('js/bar.js', 'js/foo.js', '**/*.{js,style}');
    expected = ['js/bar.js', 'js/foo.js', 'style/baz.style', 'style/qux.style'];
    test.deepEqual(actual, expected, 'specifically-specified-up-front file order should be maintained.');

    actual = globule.find('**/*.{js,style}', '!style/qux.style', 'style/qux.style');
    expected = ['style/baz.style', 'js/bar.js', 'js/foo.js', 'style/qux.style'];
    test.deepEqual(actual, expected, 'if a file is excluded and then re-added, it should be added at the end.');

    actual = globule.find('js/foo.js', '**/*.{js,style}', '!style/qux.style', 'style/qux.style');
    expected = ['js/foo.js', 'style/baz.style', 'js/bar.js', 'style/qux.style'];
  ...
  'exclusion': function(test) {
    ...
    test.deepEqual(globule.find(['!js/foo.js', '**/*.js']),
      ['js/bar.js', 'js/foo.js'],
      'inclusion / exclusion order matters');
    test.deepEqual(globule.find(['**/*.js', '**/*.style', '!js/bar.js', '!style/baz.style']),
      ['js/foo.js','style/qux.style'],
      'multiple exclusions should be removed from the set');
    test.deepEqual(globule.find(['**/*.js', '**/*.style', '!**/*.style']),
      ['js/bar.js', 'js/foo.js'],
      'excluded wildcards should be removed from the matched set');
    test.deepEqual(globule.find(['js/bar.js', 'js/foo.js', 'style/baz.style', 'style/qux.style', '!**/b*.*']),
      ['js/foo.js', 'style/qux.style'],
      'different pattern for exclusion should still work');
    test.deepEqual(globule.find(['js/bar.js', '!**/b*.*', 'js/foo.js', 'style/baz.style', 'style/qux.style']),
      ['js/foo.js', 'style/baz.style', 'style/qux.style'],
      'inclusion / exclusion order matters');
    test.done();
  },
  ...
  'options.filter': function(test) {
    ...
    test.deepEqual(globule.find('js', 'style', {filter: 'isFile'}), [], 'should fail to match.');
  ...
  'options.matchBase': function(test) {
    ...
    test.deepEqual(globule.find('*.js', '*.style', {matchBase: true}),
      ['js/bar.js', 'js/foo.js', 'style/baz.style', 'style/qux.style'],
      'matchBase option should be passed through to minimatch.');
    test.done();
  },
